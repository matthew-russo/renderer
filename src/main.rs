...
impl HelloTriangleApplication {
    pub fn initialize() -> Self {
        ...
        let ui_layer = Arc::new(Mutex::new(UiLayer::new(&graphics_queue, &device, &render_pass, dimensions)));

        let render_layers: Vec<Arc<Mutex<RenderLayer>>> = vec![
            Arc::new(Mutex::new(SceneLayer::new(&graphics_queue, &device, &render_pass, dimensions))),
            ui_layer.clone(),
        ];
